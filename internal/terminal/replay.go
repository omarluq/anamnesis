...
// the transcript once the scripted investigation completes.
...
// newReplayController returns a replayController that replays script, waiting pace
// before each event: a leading beat before the first line, then the same gap
// before every subsequent event. Pass a zero pace to emit each event as fast as
// the shell drains the channel, which keeps offline smoke runs instant.
...
// replay streams the script onto out, stamping each event with runID so the shell
// loop can gate stale work, and closes out once the script is exhausted or ctx is
// canceled.
