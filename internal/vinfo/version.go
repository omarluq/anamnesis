...
func String() string {
	info, _ := debug.ReadBuildInfo() // nil when build info is unavailable; stringFromVersion handles nil
...
func (metadata buildMetadata) withBuildInfoFallback(info *debug.BuildInfo) buildMetadata {
	...
	if metadata.buildDate == defaultBuildDate {
		buildTime := buildInfoSetting(info.Settings, buildInfoTimeKey)
		metadata.buildDate = metadataPart(buildTime, defaultBuildDate)
	}
